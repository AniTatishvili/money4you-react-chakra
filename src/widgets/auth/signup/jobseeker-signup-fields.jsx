...
import { JobseekerResumeUploadInput } from "../ui";

import { useLocation } from "react-router-dom";

import { AtSignIcon, LockIcon } from "@chakra-ui/icons";

export const JobseekerSignupFields = ({ isSubmitting, isValid, dirty }) => {
  const { t, i18n } = useTranslation(["common"]);

  const { phone, terms_status, social_title, gender, country } = useAppSelector((state) => state.signup.data);
  ...
  return (
    <>
      <Flex justifyContent="center" wrap={{ base: "wrap", md: "nowrap" }} gap={4}>
        ...
        <Flex flexDir="column" gap={4} w="100%" maxW="280px">
          <Box ref={(ref) => setRef("resumeRef", ref, t("forms:UPLOAD_RESUME"))}>
            <JobseekerResumeUploadInput />{" "}
          </Box>
          ...
          <Box ref={(ref) => setRef("signupRef", ref, t("common:USER.AUTH.SIGNUP"))}>
            <PButton
              ...
              maxW="280px"
              isDisabled={
                isSubmitting || !(resume_file && terms_status && phone && social_title && gender && country && isValidPhoneNumber(phone) && isValid && dirty)
              }
